...
const Dashboard: React.FC = () => {
  ...
  const analyzeJsonFiles = async () => {
    ...
    const apiKeys = [
      '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',
      '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',
      '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'
    ];
    ...
    try {
      ...
      const makeApiRequest = async (prompt, apiKey, retryCount = 0) => {
        try {
          ...
          console.error(`API request failed with key ${retryCount + 1}:`, error);
          if (retryCount < apiKeys.length - 1) {
            return makeApiRequest(prompt, apiKeys[retryCount + 1], retryCount + 1);
          }
          throw error;
        }
      };
      ...
      const prompts = [
        {
          name: 'SCA Analysis',
          content: `Analyze the following Failed SCA Results for CIS compliance issues:

${JSON.stringify(scaData, null, 2)}

Provide:
1. Summary of critical CIS compliance failures
2. Risk level assessment
3. Simple remediation steps for each failed rule
4. Priority ranking
...
Format in clear, actionable language.`
        },
        {
          ...
          content: `Analyze the following network ports data for security risks:

${JSON.stringify(networkData, null, 2)}

Provide:
1. Analysis of open ports and security risks
2. Potentially dangerous services
...
Format in clear, actionable language.`
        },
        {
          ...
          content: `Analyze the following process data for security concerns:

${JSON.stringify(processData, null, 2)}

Provide:
1. Analysis of running processes
2. Security concerns with current processes
3. Resource usage analysis
4. Recommendations for process security
...
Format in clear, actionable language.`
      ...
      const analysisResults = await Promise.allSettled(
        prompts.map((prompt, index) => 
          makeApiRequest(prompt.content, apiKeys[index % apiKeys.length])
        )
      );
      ...
      prompts.forEach((prompt, index) => {
        combinedResult += `## ${prompt.name}\n\n`;
        
        if (analysisResults[index].status === 'fulfilled') {
          combinedResult += analysisResults[index].value + '\n\n';
        } else {
          combinedResult += `Analysis failed: ${analysisResults[index].reason.message}\n\n`;
        }
        
        combinedResult += '---\n\n';
      });

      const summaryPrompt = `Based on the following three security analyses, provide a consolidated executive summary with overall risk assessment and top 5 priority actions:

${combinedResult}

Provide a brief executive summary focusing on:
1. Overall security posture rating (Critical/High/Medium/Low)
2. Top 5 most critical issues requiring immediate attention
3. Quick wins for improving security
4. Long-term security recommendations`;

      try {
        const executiveSummary = await makeApiRequest(summaryPrompt, apiKeys[0]);
        combinedResult = `# Executive Summary\n\n${executiveSummary}\n\n---\n\n${combinedResult}`;
      } catch (error) {
        console.error('Executive summary generation failed:', error);
        combinedResult = `# Executive Summary\n\nExecutive summary generation failed. See detailed analysis below.\n\n---\n\n${combinedResult}`;
      }
      ...
      setAnalysisResult(`Comprehensive analysis failed: ${error.message}\n\nThis could be due to:\n1. All API keys exhausted or invalid\n2. Network connectivity issues\n3. Missing JSON files\n4. API rate limits exceeded\n\nPlease verify your API keys and file paths.`);
    } finally {
